...
def select_specific_anchors(objects: list[dict[str, Any]], reviews: list[dict[str, Any]], min_confidence: float) -> list[dict[str, Any]]:
    by_id = {int(row["object_id"]): row for row in reviews}
    labels = {"floor", "wall", "grass", "roof", "ceiling", "stair"}
    selected = []
    for obj in objects:
        parsed = (by_id.get(int(obj["object_id"]), {}).get("parsed") or {})
        if (
            parsed.get("controlled_label") in labels
            and bool(parsed.get("is_surface_fragment"))
            and float(parsed.get("confidence") or 0.0) >= min_confidence
        ):
            selected.append({
                **obj,
                "first_pass_label": str(parsed["controlled_label"]),
                "first_pass_confidence": float(parsed["confidence"]),
                "first_pass_description_zh": str(parsed.get("description_zh") or ""),
            })
    return selected
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("generic", "specific_anchor"), default="generic")
    args = parser.parse_args()

    objects, reviews = read_jsonl(args.objects_jsonl), read_jsonl(args.review_jsonl)
    selected = select_candidates(objects, reviews, args.min_confidence) if args.mode == "generic" else select_specific_anchors(objects, reviews, args.min_confidence)
    ...
    report = {"candidates": len(selected), "min_confidence": args.min_confidence, "mode": args.mode}
