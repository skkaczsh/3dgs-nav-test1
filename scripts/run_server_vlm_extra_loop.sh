...
SOURCE_SAM_MASKS_DIR="${SOURCE_SAM_MASKS_DIR:-${SAM_MASKS_DIR}}"
...
PY
  ...
  if [[ "${count}" -gt 0 ]]; then
    ...
    if ! (
      ...
      export EXISTING_SAM_DIR="${SOURCE_SAM_MASKS_DIR}"
      export PART0="${SOURCE_SAM_MASKS_DIR}"
      export PART1="${SOURCE_SAM_MASKS_DIR}"
