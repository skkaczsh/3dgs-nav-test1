...
mkdir -p "${WORK_DIR}"/{onnx,engines,cpp,logs,build}
...
print("onnxsim", onnxsim.__version__)
...
onnx.save(model, "/root/epfs/sam2_tensorrt/onnx/tiny_conv.onnx")
print("/root/epfs/sam2_tensorrt/onnx/tiny_conv.onnx")
PY
"${PYTHON_BIN}" "${WORK_DIR}/onnx/make_tiny_conv_onnx.py"

echo "[4/5] Build and benchmark tiny TensorRT engine"
...
echo "[5/5] OK: SAM2 TensorRT base environment is ready"
