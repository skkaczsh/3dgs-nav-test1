...
When the gate JSON references its visual acceptance source, the launcher
recomputes the gate in memory and blocks stale cached gate files.  This prevents
semantic fusion from running against a promotion decision generated by older
rules or older visual-review data.
